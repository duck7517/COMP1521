...
reveal_grid__body:
        ...
        li      $t0, 0          # row = 0
        li      $t1, 0          # col = 0
        la      $t2, grid       # $t2 = &grid[0][0]
        
reveal_grid__body_col_loop:
        bge     $t1, N_COLS, reveal_grid__body_row_loop
        
        mul     $t3, $t0, N_COLS        #calculate current position in array
        add     $t3, $t3, $t1
        add     $t4, $t2, $t3           # $t4 contains address of curr
        lb      $t5, ($t4)
        or      $t5, $t5, IS_RVLD_MASK  # update curr cell to be revealed
        sb      $t5, ($t4)
        
        addi    $t1, $t1, 1
        b       reveal_grid__body_col_loop
        
        
reveal_grid__body_row_loop:        
        bge     $t0, N_ROWS, reveal_grid__epilogue      #repeat for each row
        add     $t0, $t0, 1
        li      $t1, 0
        b       reveal_grid__body_col_loop
...
place_bombs__loop:
        bge     $s0, $s1, place_bombs__epilogue
...
mark_cell:
        ...
        #
        # Frame:    $ra, $s0, $s1
        # Uses:     $s0, $s1, $t0, $t1, $t2, $t3, $t4, $t5, $t7
        # Clobbers: $t0, $t1, $t2, $t3, $t4, $t5, $t7
        #
        # Locals:
        #   - [...]
        #
...
mark_cell__body:
        ...
        mul     $t1, $s1, N_COLS
        add     $t1, $t1, $s0
        add     $t1, $t1, $t0           # calculate address of cell in $t1
        lb      $t2, ($t1)              # extract value of cell into $t2
        
        lw      $t3, debug_mode         #debug_mode, if true, dont check RVLD
        
        beq     $t3, FALSE, mark_cell__body_check_revealed      #if not debug 
        b       mark_cell__epilogue      
        
mark_cell__body_not_revealed:
        and     $t4, $t2, IS_MRKD_MASK                          #check if marked
        beq     $t4, IS_MRKD_MASK, mark_cell__body_is_marked
        
mark_cell__body_not_marked:
        or      $t2, $t2, IS_MRKD_MASK
        sb      $t2, ($t1)
...
mark_cell__body_is_marked:
        li      $t5, IS_MRKD_MASK       #flip is_mrkd bit
        not     $t5, $t5
        and     $t2, $t2, $t5
        sb      $t2, ($t1)
...
mark_cell__body_check_revealed:
        and     $t4, $t2, IS_RVLD_MASK                  #extract rvld bits
        beq     $t4, IS_RVLD_MASK, mark_cell__body_is_revealed  
        b       mark_cell__body_not_revealed
        
mark_cell__body_is_revealed:
        la      $a0, mark_error                         #cannot rvl alredy rvld
        li      $v0, 4
        syscall
...
reveal_cell__body:
        ...
        add     $t1, $t1, $s0
        add     $t1, $t1, $t0           # calculate address of cell in $t1
        lb      $t0, ($t1)              # extract value of cell into $t0
        
        li      $t2, IS_BOMB_MASK
        and     $t3, $t0, $t2          
        beq     $t3, IS_BOMB_MASK, reveal_cell__body_lose       #cell is bomb

reveal_cell__body_check_debug:
        lw      $t3, debug_mode                 #debug_mode, if true, end
        beq     $t3, FALSE, reveal_cell__body_check_mrkd      #if not debug 
        b       reveal_cell__epilogue
        
reveal_cell__body_check_mrkd:
        li      $t2, IS_MRKD_MASK
        and     $t3, $t0, $t2          
        beq     $t3, IS_MRKD_MASK, reveal_cell__body_mrkd       #cell is mrkd
        b       reveal_cell__body_check_rvld
        
reveal_cell__body_check_rvld:
        li      $t2, IS_RVLD_MASK
        and     $t3, $t0, $t2          
        beq     $t3, IS_RVLD_MASK, reveal_cell__body_rvld       #cell is rvld
        b       reveal_cell__body_reveal

reveal_cell__body_reveal:
        li      $t2, VALUE_MASK
        beq     $t2, 0, reveal_cell__body_clear_surr
        or      $t0, $t0, IS_RVLD_MASK          #grid[row][col] |= IS_RVLD_MASK
        b       reveal_cell__body_revealed

reveal_cell__body_clear_surr:
        addiu   $sp, $sp, -8
        sw      $a0, 12($sp)
        sw      $a1, 16($sp)
...
reveal_cell__body_revealed:
        la      $t4, cells_left                 #cells_left --
        lw      $t5, cells_left
        sub     $t5, $t5, 1
        sw      $t5, ($t4)
        beq     $t5, 0, reveal_cell__body_win
        b       reveal_cell__epilogue

reveal_cell__body_mrkd:
        la      $a0, reveal_error
        li      $v0, 4
        syscall
        b       reveal_cell__body_check_rvld
        
reveal_cell__body_rvld:
        la      $a0, already_revealed
        li      $v0, 4
        syscall
        b       reveal_cell__epilogue
        
reveal_cell__body_win:
        li      $t0, WIN                       #trigger win game_state
        sw      $t0, game_state
        b       reveal_cell__epilogue

reveal_cell__body_lose:
        li      $t0, LOSE                       #trigger lose game_state
        sw      $t0, game_state
        b       reveal_cell__epilogue
...
#        move    $s0, $a0                #score in $s0
#        la      $s1, high_score         #address of high_score in $s1
#        lw      $s2, high_score         #value of high_score in $s2
#        ble     $s0, $s2, update_highscore__body_fail
#        
#update_highscore__body_new_score:
#        move    $s2, $s0
#        sw      $s1, 0($s1)
#        
#        li      $t0, 0                  #i = 0
#        li      $t1, '\0'
#        
#update_highscore__body_new_user:
#        addi    $t2, $t0, N_INT_BYTES  
#        lw      $t3, ($t2)
#        beq     $t3, $t1, update_highscore__body_new_user_end
#        
#        
#        
#        b       update_highscore__body_new_user
#       
#update_highscore__body_new_user_end:       
#     
#update_highscore__body_fail:
#        li      $v0, FALSE
